...
const WarehouseDashboard = () => {
  ...
  // alert("screen width: "+ window.innerWidth+ " && height: "+ window.innerHeight)
  ...
  return (
      ...
      <motion.div
        ...
        animate="visible"
        exit="exit"
        className="grid grid-cols-1 sm:grid-cols-5 gap-6 mb-8"
      >
        {fabrics.map((fabric, index) => (
          <motion.div
            key={fabric ? `${fabric.ITEM_CODE}-${pageInfo.page}` : `empty-${index}-${pageInfo.page}`}
            ...
            <div className="flex flex-col h-full">
              ...
              <div className="bg-teal-600 text-white p-4">
                ...
                <p className="text-xm truncate">
                  Total: {fabric.INVOICE_NO} {fabric.UOM}
                </p>
              </div>
 
              {/* Card Content */}
              <div className="p-4 max-h-60 overflow-y-auto flex-1">
                <table className="w-full">
                  <thead className="bg-gray-100 sticky top-0">
                    <tr>
                      <th className="p-2 text-left text-xs">PO No</th>
                      <th className="p-2 text-left text-xs">Location</th>
                      <th className="p-2 text-left text-xs">Invoice No</th>
                  ...
                  <tbody>
                    {fabric.locations.map((location, i) => (
                      <tr key={i} className="border-t">
                        <td className="p-2 text-sm">{location.FAB_PO_NO}</td>
                        <td className="p-2 text-sm">{location.FULL_LOCATION}</td>
                        <td className="p-2 text-sm">{location.INVOICE_NO || null}</td>
                        <td className="p-2 text-sm">{location.QTY}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
              ...
              <div className="bg-gray-200/50 p-3 text-sm">
                ...
                <p>Buyer: {fabric.BUYER_NAME || "N/A"}</p>
              </div>
            </div>
          </motion.div>
        ))}
